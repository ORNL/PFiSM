 ...
 ************************************************************************/
void PFModelARK::initializeLevelData(
    ...
    const bool allocate_data)
{
   ...
   // Data that is not managed inside the SAMRAI ARKODESolver class
   // and that must be set on the level should be initialized here
}

void PFModelARK::resetHierarchyConfiguration(
    const std::shared_ptr<hier::PatchHierarchy>& hierarchy,
...
void PFModelARK::applyGradientDetector(
    const std::shared_ptr<hier::PatchHierarchy>& hierarchy,
 ...
 ***********************************************************************/
void PFModelARK::setPhysicalBoundaryConditions(
...
void PFModelARK::preprocessRefine(hier::Patch& fine, const hier::Patch& coarse,
...
void PFModelARK::postprocessRefine(hier::Patch& fine, const hier::Patch& coarse,
                                   const hier::Box& fine_box,
                                   const hier::IntVector& ratio)
{
   NULL_USE(fine);
 ...
 ************************************************************************/
void PFModelARK::preprocessCoarsen(hier::Patch& coarse, const hier::Patch& fine,
...
void PFModelARK::postprocessCoarsen(hier::Patch& coarse,
...
void PFModelARK::evaluateRHSPhase(
...
void PFModelARK::evaluateRHSTemperature(
    std::shared_ptr<hier::PatchHierarchy> hierarchy,
                                       ...
                                       solv::SundialsAbstractVector* y_dot)
{
   ...
   // Compute rhs for phase first, since it is used in rhs for temperature
   // trying to compute PF explicitly
   for (int ln = hierarchy->getFinestLevelNumber(); ln >= 0; --ln) {
      std::shared_ptr<hier::PatchLevel> level(hierarchy->getPatchLevel(ln));
      ...
      for (hier::PatchLevel::iterator ip(level->begin()); ip != level->end();
         ...
         std::shared_ptr<pdat::CellData<double> > temperature(
             SAMRAI_SHARED_PTR_CAST<pdat::CellData<double>, hier::PatchData>(
                 patch->getPatchData(d_temperature_scr_id)));
         std::shared_ptr<pdat::CellData<double> > rhsimp(
             SAMRAI_SHARED_PTR_CAST<pdat::CellData<double>, hier::PatchData>(
   ...
   for (int ln = hierarchy->getFinestLevelNumber(); ln >= 0; --ln) {
      ...
      for (hier::PatchLevel::iterator ip(level->begin()); ip != level->end();
         ...
         // use velocity 0. since velocity term is in explicit part
         SAMRAI_F77_FUNC(comprhs3d, COMPRHS3D)
         (ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2), dx,
          y->getPointer(), y->getGhostCellWidth()[0], d_temperature_diffusion,
                                       ...
                                       solv::SundialsAbstractVector* y_dot)
{
   ...
   for (int ln = hierarchy->getFinestLevelNumber(); ln >= 0; --ln) {
      ...
      for (hier::PatchLevel::iterator ip(level->begin()); ip != level->end();
         ...
         std::shared_ptr<pdat::CellData<double> > temperature(
             SAMRAI_SHARED_PTR_CAST<pdat::CellData<double>, hier::PatchData>(
                 patch->getPatchData(d_temperature_scr_id)));
         std::shared_ptr<pdat::CellData<double> > rhsex(
             SAMRAI_SHARED_PTR_CAST<pdat::CellData<double>, hier::PatchData>(
                 patch->getPatchData(y_dot_samvect->getComponentDescriptorIndex(
                     d_phase_component))));
         assert(phase);
         ...
         // set explicit term to 0. for phase
         SAMRAI_F77_FUNC(comprhsex3d, COMPRHSEX3D)
         (ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2), dx,
   ...
   // now compute rhs for temperature
   // trying to compute the temperaure implicitly
   for (int ln = hierarchy->getFinestLevelNumber(); ln >= 0; --ln) {
      std::shared_ptr<hier::PatchLevel> level(hierarchy->getPatchLevel(ln));
...
void PFModelARK::initializeSolvers(
    const std::shared_ptr<hier::PatchHierarchy>& hierarchy)
{
...
void PFModelARK::PrecondSetPhase(
    std::shared_ptr<hier::PatchHierarchy> hierarchy, int y_indx,
    const double gamma)
{
...
void PFModelARK::PrecondSetTemperature(const double gamma)
{
   assert(d_FAC_solver_temperature);
...
bool PFModelARK::PrecondSolveTemperature(
...
bool PFModelARK::PrecondSolvePhase(
    std::shared_ptr<hier::PatchHierarchy> hierarchy, int r1_indx, int z1_indx,
    double gamma)
{
   assert(d_FAC_solver_phase);
 ...
 ************************************************************************/
void PFModelARK::setupSolutionVector(
    std::shared_ptr<hier::PatchHierarchy> hierarchy)
{
...
/*************************************************************************
 * Set initial conditions for temperature and phase fields in ARKODE solver
 *************************************************************************/
void PFModelARK::setInitialConditions()
{
   ...
   int temp_id =
       d_FAC_solver_phase
           ? init_samvect->getComponentDescriptorIndex(d_temperature_component)
           : d_temperature_cur_id;
 ...
 *************************************************************************/
double PFModelARK::computeSolidFraction(
    const std::shared_ptr<hier::PatchHierarchy>& hierarchy)
{
   assert(d_vol_id >= 0);
 ...
 *************************************************************************/
void PFModelARK::getFromInput(std::shared_ptr<tbox::Database> input_db,
 ...
 *************************************************************************/
void PFModelARK::getFromRestart()
{
 ...
 *************************************************************************/
void PFModelARK::registerVisItDataWriter(
...
void PFModelARK::printClassData(ostream& os) const
{
   fflush(stdout);
   os << "ptr PFModelARK = " << (PFModelARK*)this << endl;
